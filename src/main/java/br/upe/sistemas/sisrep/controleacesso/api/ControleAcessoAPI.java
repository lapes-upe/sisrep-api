...
public class ControleAcessoAPI {
  ...
  public ResponseEntity<List<UsuarioVO>> listarUsuarios() {
    return ResponseEntity
        .ok(ctrlAcessoServico.listarUsuarios().stream().map(e -> convert(e)).collect(Collectors.toList()));
  }
  ...
  public ResponseEntity<UsuarioVO> incluir(@Valid @RequestBody UsuarioVO usuarioVO) {
    ...
    URI uri = URI
        .create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/controleacesso/usuario").toUriString());
  ...
  public ResponseEntity<UsuarioVO> alterar(@PathVariable Long id, @Valid @RequestBody UsuarioVO usuarioVO) {
  ...
  public ResponseEntity<PerfilVO> incluir(@Valid @RequestBody PerfilVO perfilVO) {
    URI uri = URI
        .create(ServletUriComponentsBuilder.fromCurrentContextPath().path("/api/controleacesso/perfil").toUriString());
  ...
  public ResponseEntity<PerfilVO> alterar(@PathVariable Long id, @Valid @RequestBody PerfilVO perfilVO) {
  ...
  public ResponseEntity<List<PerfilVO>> listarPerfis() {
    return ResponseEntity
        .ok(ctrlAcessoServico.listarPerfis().stream().map(e -> convert(e)).collect(Collectors.toList()));
  }
  ...
  private Usuario convert(UsuarioVO vo) {
    ...
    Usuario usuario = Usuario.builder().id(vo.getId()).nome(vo.getNome()).email(vo.getEmail()).cpf(vo.getCpf())
        .token(vo.getToken()).ativo(vo.getAtivo()).build();

    if (vo.getPerfis() != null && !vo.getPerfis().isEmpty()) {
      usuario.setPerfis(new ArrayList<>());

      vo.getPerfis().stream()
          .forEach(p -> usuario.getPerfis().add(Perfil.builder().id(p.getId()).nome(p.getNome()).build()));
    }
  ...
  private UsuarioVO convert(Usuario entidade) {
    ...
    UsuarioVO usuario = UsuarioVO.builder().id(entidade.getId()).nome(entidade.getNome()).email(entidade.getEmail())
    ...
    if (entidade.getPerfis() != null && !entidade.getPerfis().isEmpty()) {
      ...
      entidade.getPerfis().stream()
          .forEach(p -> usuario.getPerfis().add(PerfilVO.builder().id(p.getId()).nome(p.getNome()).build()));
    }
  ...
  private PerfilVO convert(Perfil entidade) {
    return PerfilVO.builder().id(entidade.getId()).nome(entidade.getNome()).icone(entidade.getIcone()).build();
  }
}
